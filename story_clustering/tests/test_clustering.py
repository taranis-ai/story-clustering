...
def test_create_corpus():
    ...
    cluster = Cluster()
    corpus = cluster.create_corpus(news_item_list)
...
def test_initial_clustering():
    ...
    cluster = Cluster()
    clustering_results = cluster.initial_clustering(news_item_list)
...
def test_incremental_clustering():
    ...
    cluster = Cluster()
    clustering_results = cluster.incremental_clustering_v2(news_item_list, clustered_news_item_list)
    print(clustering_results)
...
def test_dump_corpus():
    from story_clustering.clustering import Cluster
    from story_clustering.eventdetector import extract_events_from_corpus
