...
class Louvain:
    ...
    async def predict(self, stories: list[dict]) -> dict[str, Any]:
        if all(len(story["news_items"]) == 1 for story in stories):
            clustering_results = await asyncio.to_thread(self.model.initial_clustering, stories)
        ...
        clustering_results = await asyncio.to_thread(self.model.incremental_clustering_v2, to_cluster, already_clustered)
