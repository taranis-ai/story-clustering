...
def test_create_corpus(story_list):
    ...
    cluster = Cluster()
    corpus = cluster.create_corpus(story_list)
    assert len(corpus.docs) == 8
    assert corpus.docs["1"].title == "Test News Item 13"
    assert "cve-2021-5678" in corpus.docs["2"].keywords.keys()


def test_initial_clustering(story_list):
    from story_clustering.clustering import Cluster

    cluster = Cluster()
    clustering_results = cluster.initial_clustering(story_list)
...
def test_dump_corpus(story_list):
    from story_clustering.clustering import Cluster
